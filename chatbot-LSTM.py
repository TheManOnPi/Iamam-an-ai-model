#!/usr/bin/env python3
#    This file is licensed under the GNU Affero General Public License v3.0 (https://www.gnu.org/licenses/agpl-3.0.en.html)
#    This file is the IAMAM ai system.
#    Copyright (C) 2026  TheManOnPi
...
# legal
def legal_check():
    lp("i", "Licensed under GNU AGPL v3.0 | https://github.com/TheManOnPi/Iamam-an-ai-model/blob/main/LICENSE")
    
    if "--accept-license" not in sys.argv:
        lp("i", "Run with --accept-license to skip this prompt.")
        accept = input("Do you accept the AGPLv3 terms? [y/N]: ").strip().lower()
        if accept != 'y':
            lp("f", "License acceptance required.")
            sys.exit(exitCodes["FATAL"])
...
def main():
    ...
    if os.path.exists(model_path) and not RETRAIN:
        ...
        context = att([dec_out, enc_out], mask=[dec_mask, enc_mask])
    ...
    print(f"Tokeniser size: {len(tokenizer.word_index)}")
    # Train
    if RETRAIN:
...
def build_inference_models(model):
    lp("i", "Building compiled inference models with Masking Support...")
    ...
    dec_bn_layer = model.get_layer("dec_bn") 
    dec_dense_layer = model.get_layer("dec_dense")

    # 2. Reconstruct Encoder (Now exporting the mask)
    enc_emb_out = enc_emb_layer(enc_inp)
    enc_mask = enc_emb_layer.compute_mask(enc_inp) # <--- Capture mask
    ...
    # inf_enc now returns the sequence, states, AND the mask
    ...
    e_mask_in = Input(shape=(MAX_LEN,), dtype='bool', name="inf_enc_mask") # <--- Mask input
    d_h_in = Input(shape=(LATENT_DIM*2,), name="inf_dec_h")
    d_c_in = Input(shape=(LATENT_DIM*2,), name="inf_dec_c")

    # Get decoder embedding and its mask
    d_emb = dec_emb_layer(d_token)
    d_mask = dec_emb_layer.compute_mask(d_token) # <--- Current token mask

    dec_out, d_h, d_c = dec_lstm_layer(d_emb, initial_state=[d_h_in, d_c_in])
    
    # FIX: Pass both the decoder mask and the encoder mask to Attention
...
def reply(text, max_decode_len=MAX_LEN, temp=0.7, top_k=35, rep_penalty=1.3):
    global INF_ENCODER, INF_DECODER, TOK, WORD2IDX, IDX2WORD
    ...
    for i in range(max_decode_len):
        logits_tensor, h, c = fast_decode_step(current_token, e_seq, h, c, INF_DECODER, e_mask=e_mask)
